  ...
  return async (data) => {
    try {
      ...
      if (error.response.data?.error !== 0) {
        ...
        showToast.error("error occurred while creating faq.");
      }
      return false;
    }
  };
...
const useFetchFaqs = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${BASE_SERVER_URL}/admin/faqs`, config);
  ...
  return { faqs, refetch: fetchData, loading };
};
...
export { useAddFaqs, useFetchFaqs, useGetFaq, useUpdateFaq, useDeleteFaq };
