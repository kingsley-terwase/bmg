...
import { useGetPortfolio } from "../../../Hooks/Dashboard/portfolios";
import { formatDate } from "../../../utils/functions";
import { BASE_IMAGE_URL } from "../../../Config/paths";

const SingleConsultationModal = ({ open, onClose, consultId }) => {
  const navigate = useNavigate();
  const { data, loading, getPortfolio } = useGetPortfolio();
  console.log(" ID:", consultId);

  useEffect(() => {
    if (open && consultId) {
      getPortfolio(consultId);
    }
  }, [open, consultId]);

  const handleEdit = () => {
    console.log("Edit category:", consultId);
    navigate(`/dashboard/admin/edit/sub-categories/${consultId}`);
    onClose();
  };

  const handleDelete = () => {
    if (window.confirm(`Are you sure you want to delete "${data?.name}"?`)) {
      console.log("Delete category:", consultId);
      onClose();
    }
  };
            ...
            >
              ...
              >
                ...
                <Stack direction="row" spacing={1} alignItems="center">
                  ...
                  {data?.total_services && (
                    <Chip
                      label={`${data?.total_services} Services`}
                      size="small"
                      sx={{
...
export default SingleConsultationModal;
